...
export default function GitHubWidget({ block, socialProfile, loading = false, error = null, onRetry }) {
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: '0.85rem', color: '#0969DA', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {repoName}
            </span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            ...
            <span style={{ fontSize: '0.65rem', color: '#57606A', background: '#F6F8FA', border: '1px solid #D0D7DE', padding: '1px 6px', borderRadius: 10, fontWeight: 600 }}>
              Public
            </span>
          </div>
        </div>
        ...
        {repo.description && (
          <p style={{ margin: 0, fontSize: '0.76rem', color: '#57606A', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', lineHeight: 1.35 }}>
            {repo.description}
          </p>
        )}
  ...
  return (
    <SocialWidgetLayout
      ...
      accentColor="#24292E"
      recentContent={recentRepos}
      renderRecentItem={renderRepoItem}
      loading={loading}
      error={error}
      onRetry={onRetry}
    />
